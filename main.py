...
if __name__ == '__main__':
    ...
    #import pdb
    #pdb.set_trace()
    for epoch in tqdm.tqdm(range(1, args.max_epoch + 1)):
        # lr_scheduler.step()
        ...
        for i, batch in enumerate(train_loader, 1):
            ...
            results, results2 = dense_predict_network(feat_query, feat_shot, args)
            results = torch.cat(results, dim=0)  # Q x S
            label = torch.arange(args.way).repeat(args.query).long().to('cuda')
            ###
            support_labels = torch.arange(args.way).repeat(args.shot).long().to('cuda')
            label2 = torch.cat((label, support_labels), dim = 0)
            ...
            one_hot2 = torch.zeros_like(results2).scatter(1, label2.view(-1, 1), 1)
            one_hot2 = one_hot2 * (1 - eps) + (1 - one_hot2) * eps / (args.way - 1)
            log_prb2 = F.log_softmax(results2, dim=1)

            loss1 = -(one_hot * log_prb).sum(dim=1)
            loss2 = -(one_hot2 * log_prb2).sum(dim=1)
